...
class ModelInpaint():
    ...
    def build_inpaint_graph(self):
        ...
        with self.graph.as_default():
            ...
            self.go_symm = tf.map_fn(lambda img: tf.image.flip_left_right(img), self.go)

            self.symmetry_loss = tf.reduce_sum(
                tf.contrib.layers.flatten(
                    tf.abs(self.go_symm -self.go)), 1
            )

            self.inpaint_loss = self.context_loss + self.l*self.perceptual_loss + 0.1*self.symmetry_loss
