...
def train():
    ...
    model = Network(x, mask, local_x, global_completion, local_completion, is_training, batch_size=BATCH_SIZE, config=config)
    ...
    while(True):
        ...
        log.debug('epoch: {}'.format(sess.run(epoch)))
        ...
        if sess.run(epoch) <= PRETRAIN_EPOCH:
            ...
            log.debug('Completion loss: {}'.format(g_loss_value))
        ...
        # Discrimitation
            ...
            log.debug('Completion loss: {}'.format(g_loss_value))
            log.debug('Discriminator loss: {}'.format(d_loss_value))
            print('Completion loss: {}'.format(g_loss_value))
