    ...
    def generator(self, x, is_training):
        with tf.variable_scope('generator'):
            ...
            with tf.variable_scope('conv6'):
                x = conv_layer(x, [3, 3, 256, 256], 1)
                x = tf.nn.relu(x)
            mask_s = resize_mask_like(self.mask_bbox, x)
            ...
            with tf.variable_scope('pmconv1'):
                x = conv_layer(x_now, [5, 5, 3, 64], 1)
                x = tf.nn.relu(x)
            with tf.variable_scope('pmconv2'):
                x = conv_layer(x, [3, 3, 64, 128], 2)
                x = tf.nn.relu(x)
            with tf.variable_scope('pmconv3'):
                x = conv_layer(x, [3, 3, 128, 128], 1)
                x = tf.nn.relu(x)
            with tf.variable_scope('pmconv4'):
                x = conv_layer(x, [3, 3, 128, 256], 2)
                x = tf.nn.relu(x)
            with tf.variable_scope('pmconv5'):
                x = conv_layer(x, [3, 3, 256, 256], 1)
                x = tf.nn.relu(x)
            with tf.variable_scope('pmconv6'):
                x = conv_layer(x, [3, 3, 256, 256], 1)
                x = tf.nn.relu(x)

            x = contextual_attention(x, x, mask_s, 3, 1, rate=2)

            with tf.variable_scope('pmconv9'):
                x = conv_layer(x, [3, 3, 256, 256], 1)
                x = tf.nn.relu(x)
            with tf.variable_scope('pmconv10'):
                x = conv_layer(x, [3, 3, 256, 256], 1)
                x = tf.nn.relu(x)
